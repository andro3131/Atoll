# -*- coding: utf-8 -*-
###########################################################
#   Skripta vrne upraviÄenost bazne postaje. Inputi so:
...
#   2. Potem izvozimo Å¡e skupno pokrivanje po tehnologijah (GSM, LTE, NR). Naredimo ascii exporte.
#   Nato
#
#
###########################################################
...
def beri_atoll_txt_export(atoll_txt_export_file_path):
    ...
    servers: katere serverje vrne. Äe jih vrne veÄ, jih je treba specificirati v seznamu. Npr: [1,2,5] -> vrne serverje 1, 2 in 5.
    """
...
def beri_atoll_txt_export_1(atoll_txt_export_file_path):
    ...
    # preÅ¡tej loÄila v vrstici, Äe je uprabljeno \t ga spremeni v podpiÄje
    file_podpicje = []
    for i in file_ostalo:
        file_podpicje.append(i.replace("\t",";").strip("\n"))
    # PreÅ¡tej podpicja v vrsticah. Äe jih je manj kot 13 jih dodaj toliko, da jih bo 13.
    file_podp = []
...
def izracunaj_stevilke(mapa = '', lokacija = '', naredi_slike = False):
    ...
    # tocke_df['y_stot'] = (tocke_df['N'] - tocke_df['N']%100).astype(int)
    ...
    for k in scenarij:
        ...
        if (naredi_slike == True) & ((k.find('LTE') >= 0) | (k.find('NR') >=0)):
            try:
                ...
                pass
            # # 'Novi_Naslovi_best_indoor'
            # shape_tocke(df = presek1[['E','N']][(presek1['3_prim'] >= nivo_indoor) & (presek1['3_ostali'] < nivo_indoor)].drop_duplicates(), odlozisce = mapa + "Slike\\", ime_fajla = "Novi_Naslovi_best_indoor1.shp")
            # df__ = presek1__[['x_stot',  'y_stot' ,  '2_prim' , '3_prim']][(presek1__['3_prim'] >= nivo_indoor) & (presek1__['3_ostali'] < nivo_indoor)].drop_duplicates()
            # df__.columns = [0,1,2,3]
            # df_to_tiff.naredi_tif1(file_pokrivanje = df__, odlozisce = mapa + "Slike\\", tip = 'df', resolution = 25, ime = 'Novi_Naslovi_best_indoor1', treshold = nivo_indoor, x_offset = 12.5, y_offset = 12.5)
            # manipulacija_slik.transparent2(mapa = mapa + "Slike\\", slika = "Novi_Naslovi_best_indoor1.tif", odlozisce = mapa + "Slike\\")
            # os.remove(mapa + "Slike\\" + 'Novi_Naslovi_best_indoor1.tif')
            # os.rename(mapa + "Slike\\" + 'Novi_Naslovi_best_indoor1__TRANSPARENT.tif', mapa + "Slike\\" + 'Novi_Naslovi_best_indoor1.tif')

            # # 'Novi_Naslovi_best_outdoor'
            # ime__ = lokacija + "_Novi_Naslovi_best_outdoor_" + k.split("_")[1]
            # shape_tocke(df = presek1[['E','N']][(presek1['3_prim'] >= nivo_outdoor) & (presek1['3_ostali'] < nivo_outdoor)].drop_duplicates(), odlozisce = mapa + "Slike\\", ime_fajla = ime__ +"_naslovi.shp")
            # df__ = presek1__[['x_stot',  'y_stot' ,  '2_prim' , '3_prim']][(presek1__['3_prim'] >= nivo_outdoor) & (presek1__['3_ostali'] < nivo_outdoor)].drop_duplicates()
            # df__.columns = [0,1,2,3]
            ...
            # # 'Izboljsava_Naslovi_best_indoor'
            # shape_tocke(df = presek1[['E','N']][(presek1['3_prim'] >= nivo_indoor) & (presek1['3_prim'] > presek1['3_ostali'])].drop_duplicates(), odlozisce = mapa + "Slike\\", ime_fajla = "Izboljsava_Naslovi_best_indoor.shp")
            # df__ = presek1__[['x_stot',  'y_stot' ,  '2_prim' , '3_prim']][(presek1__['3_prim'] >= nivo_indoor) & (presek1__['3_prim'] > presek1__['3_ostali'])].drop_duplicates()
            # df__.columns = [0,1,2,3]
            # df_to_tiff.naredi_tif1(file_pokrivanje = df__, odlozisce = mapa + "Slike\\", tip = 'df', resolution = 25, ime = 'Izboljsava_Naslovi_best_indoor', treshold = nivo_indoor, x_offset = 12.5, y_offset = 12.5)
            # manipulacija_slik.transparent2(mapa = mapa + "Slike\\", slika = "Izboljsava_Naslovi_best_indoor.tif", odlozisce = mapa + "Slike\\")
            # os.remove(mapa + "Slike\\" + 'Izboljsava_Naslovi_best_indoor.tif')
            # os.rename(mapa + "Slike\\" + 'Izboljsava_Naslovi_best_indoor__TRANSPARENT.tif', mapa + "Slike\\" + 'Izboljsava_Naslovi_best_indoor.tif')
            ...
            # # '2nd_best_indoor'
            # shape_tocke(df = presek2[['E','N']][(presek2['3_sec'] >= nivo_indoor)].drop_duplicates(), odlozisce = mapa + "Slike\\", ime_fajla = "Izboljsava_Naslovi_second_best_indoor.shp")
            # df__ = presek2__[['x_stot',  'y_stot' ,  '2_sec' , '3_sec']][(presek2__['3_sec'] >= nivo_indoor)]
            # df__.columns = [0,1,2,3]
            # df_to_tiff.naredi_tif1(file_pokrivanje = df__, odlozisce = mapa + "Slike\\", tip = 'df', resolution = 25, ime = 'Izboljsava_Naslovi_second_best_indoor', treshold = nivo_indoor, x_offset = 12.5, y_offset = 12.5)
            # manipulacija_slik.transparent2(mapa = mapa + "Slike\\", slika = "Izboljsava_Naslovi_second_best_indoor.tif", odlozisce = mapa + "Slike\\")
            # os.remove(mapa + "Slike\\" + 'Izboljsava_Naslovi_second_best_indoor.tif')
            # os.rename(mapa + "Slike\\" + 'Izboljsava_Naslovi_second_best_indoor__TRANSPARENT.tif', mapa + "Slike\\" + 'Izboljsava_Naslovi_second_best_indoor.tif')

            # # '2nd_best_outdoor'
            # shape_tocke(df = presek2[['E','N']][(presek2['3_sec'] >= nivo_outdoor)].drop_duplicates(), odlozisce = mapa + "Slike\\", ime_fajla = "Izboljsava_Naslovi_second_best_outdoor.shp")
            # df__ = presek2__[['x_stot',  'y_stot' ,  '2_sec' , '3_sec']][(presek2__['3_sec'] >= nivo_outdoor)].drop_duplicates()
            # df__.columns = [0,1,2,3]
            # df_to_tiff.naredi_tif1(file_pokrivanje = df__, odlozisce = mapa + "Slike\\", tip = 'df', resolution = 25, ime = 'Izboljsava_Naslovi_second_best_outdoor', treshold = nivo_outdoor, x_offset = 12.5, y_offset = 12.5)
            # manipulacija_slik.transparent2(mapa = mapa + "Slike\\", slika = "Izboljsava_Naslovi_second_best_outdoor.tif", odlozisce = mapa + "Slike\\")
            # os.remove(mapa + "Slike\\" + 'Izboljsava_Naslovi_second_best_outdoor.tif')
            # os.rename(mapa + "Slike\\" + 'Izboljsava_Naslovi_second_best_outdoor__TRANSPARENT.tif', mapa + "Slike\\" + 'Izboljsava_Naslovi_second_best_outdoor.tif')
    ...
    # Lazy import + try/except: ce PNG dependencies (contextily, pip-system-certs ...) niso
    # na voljo ali OSM tile fetch fails, samo print warning - TIF+SHP+analiza so ze varni.
    if naredi_slike:
        try:
