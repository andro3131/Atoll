...
Dim ime
Dim sites
Dim f1, file1, f2, file2, f3, file3
...
Next
Set objFile = Nothing
' objDocumentAtoll.Archive   ' ZAKOMENTIRANO na planer01 - lokalne poti v SQL porusijo kolegove nocne predikcije
If Err.Number <> 0 Then
	Err.Clear
End If
'"Wait" Loop
...
For i=0 To Vrstice(file3)-1

	celica= split(split(file3, vbCrLf)(i),";")(0)
	objAtoll.LogMessage celica
	tehnologija=split(split(file3, vbCrLf)(i),";")(1)
	objAtoll.LogMessage tehnologija
	filt=split(split(file3, vbCrLf)(i),";")(2)
	...
	prediction2.Coverageconditions.Set "SERVERMARGIN",3 		' Corresponds to 0 dB
	...
	prediction2.Export objFs.GetAbsolutePathName(odlozisce  & prediction2.Name & ".TXT") ,objProjection,"RASTERTXT"
